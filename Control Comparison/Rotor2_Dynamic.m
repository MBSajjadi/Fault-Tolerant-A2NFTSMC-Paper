function [XDOT,wStar,W,fPhi,fTheta,fPsi,uFMain] =...
              Rotor2_Dynamic(t,x,u,FaultTime,alpha,gamma,Eta)
    %% A. System Parameters
    ...
    if(t>=35)
        dphi = 0.5*sin(0.5*t);
        dtheta = 0.5*cos(0.5*t);
        dpsi = 0.5*cos(-0.5*t)+0.2*sin(0.2*t);
        dx = 0.7*sin(0.7*t+pi);
        dy = dx;
        dz = dx;
    else
    ...
    if(t>=FaultTime)
       ...
       xDoubleDot = (1/m)*uThrust*u(1)-Kf*xDot/m+ufx+dx;
       yDoubleDot = (1/m)*uThrust*u(2)-Kf*yDot/m+ufy+dy;
       zDoubleDot = -g+(1/m)*uThrust*(c(theta)*c(phi))-Kf*zDot/m+ufz+dz;
